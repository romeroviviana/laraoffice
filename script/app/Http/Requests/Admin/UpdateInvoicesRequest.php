...
class UpdateInvoicesRequest extends FormRequest
{
    ...
    public function rules()
    {
        ...
        $record = \App\Invoice::find( $id );

        if ( ! empty( $product_names ) && ! empty( $record->project_id ) ) {
        ...
        $rules['invoice_no'] = 'required|numeric|unique:invoices,invoice_no,' . $this->route('invoice');
