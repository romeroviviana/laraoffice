		...
		$('#invoiceSend').click(function() {
			...
			if ( 'make-payment-pay' == action ) {				
				...
				@if ( isPluginActive('account') )
					if ( account == '' ) {
						$('#account').after('<span class="error">{{trans("custom.messages.account")}}</a>');
						$('#account').focus();
						errors++;
					}
				@endif
				if ( date == '' ) {
					$('#date').after('<span class="error">{{trans("custom.messages.date")}}</a>');
					$('#date').focus();
					errors++;
				}
				if ( amount == '' ) {
					$('#amount').after('<span class="error">{{trans("custom.messages.amount")}}</a>');
					$('#amount').focus();
					errors++;
				} else if( amount <= 0 ) {
					$('#amount').after('<span class="error">{{trans("custom.messages.amount-positive-number")}}</a>');
					$('#amount').focus();
					errors++;
				}
				if ( category == '' ) {
					$('#category').after('<span class="error">{{trans("custom.messages.category")}}</a>');
					$('#category').focus();
					errors++;
				}
				if ( paymethod == '' ) {
					$('#paymethod').after('<span class="error">{{trans("custom.messages.paymethod")}}</a>');
					$('#paymethod').focus();
					errors++;
				}
				if ( transaction_id == '' ) {
...
function printErrorMsg (msg) {
	...
	function loadEmailTemplate (invoice_id, action) {
		...
		jQuery.ajax({
	        url: baseurl + '/admin/invoices/mail-invoice',
	        type: 'POST',
	        data: {
	        	'_token': crsf_hash,
