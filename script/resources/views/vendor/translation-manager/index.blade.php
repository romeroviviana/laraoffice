...
@section('content')
<h3 class="page-title">@lang('custom.translations.title')</h3>
<div class="panel panel-default">
    <div class="panel-heading">
        @lang('custom.translations.title')
    </div>
    ...
    <p>Translations for Vuejs components</p>
    <div class="alert alert-success success-vuetranslate" style="display:none;">
        <p>Done string the translations for vuejs components!</p>
    </div>
    <form class="form-vuetranslate" method="POST" action="<?php echo route('admin.master_settings.vuetranslate') ?>" data-remote="true" role="form" data-confirm="Are you sure you want to scan you app folder? All found translation keys will be added to the database.">
        <div class="form-group">
            <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
            <button type="submit" class="btn btn-info" data-disable-with="Translating..." >Translate String for Vuejs Components</button>
        </div>
    </form>
    ...
    <?php if($group): ?>
        ...
        <hr><br>
    <p><h4>Total: <?= $numTranslations ?>, changed: <?= $numChanged ?></h4></p>
        <table class="table">
            <thead>
            <tr>
                <th width="15%">Key</th>
        ...
        <fieldset>
            ...
            <form class="form-add-locale" method="POST" role="form" action="<?php echo action('\Barryvdh\TranslationManager\Controller@postAddLocale') ?>">
                ...
                <div class="form-group">
                    ...
                    <div class="row">
                        <div class="col-sm-3">
                            <input type="text" name="new-locale" class="form-control" style="border: 1px solid darkgray;" />
                        </div>
                        <div class="col-sm-2">
...
<script>
    jQuery(document).ready(function($){
        ...
        $('#group').on('change', function(){
            var group = $(this).val();
            // alert('<?php echo action('\Barryvdh\TranslationManager\Controller@getView') ?>/'+$(this).val());
            if (group) {
        ...
        $('.form-publish').on('ajax:success', function (e, data) {
            $('div.success-publish').slideDown().fadeOut('slow');
        });

        $('.form-vuetranslate').on('ajax:success', function (e, data) {
            $('div.success-vuetranslate').slideDown().fadeOut('slow');
        });        

        $('.form-publish-all').on('ajax:success', function (e, data) {
            $('div.success-publish-all').slideDown().fadeOut('slow');
        });
