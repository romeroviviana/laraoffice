...
@section('content')
    ...
    <p>
        <a href="{{ route('admin.credit_note_payments.create') }}" class="btn btn-success"><i class="fa fa-plus"></i>&nbsp;@lang('global.app_add_new')</a>
    ...
    <p>
        <ul class="list-inline">
            <li><a href="{{ route('admin.credit_note_payments.index') }}" style="{{ request('show_deleted') == 1 ? '' : 'font-weight: 700' }}">@lang('global.app_all')</a></li> 
            @can('invoice_payment_delete')|
            <li><a href="{{ route('admin.credit_note_payments.index') }}?show_deleted=1" style="{{ request('show_deleted') == 1 ? 'font-weight: 700' : '' }}">@lang('global.app_trash')</a></li>
            @endcan
        </ul>
    </p>
...
@section('javascript') 
    <script>
        @can('invoice_payment_delete_multi')
            @if ( request('show_deleted') != 1 ) window.route_mass_crud_entries_destroy = '{{ route('admin.credit_note_payments.mass_destroy') }}'; @endif
        @endcan
        $(document).ready(function () {
            window.dtDefaultOptions.ajax = '{!! route('admin.credit_note_payments.index') !!}?show_deleted={{ request('show_deleted') }}';
