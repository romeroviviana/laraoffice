...
<header>
        ...
        <article>
            ...
            <table class="balance" style="margin-top: 0px;">
                <tr>
                    <th><span>{{trans('custom.invoices.invoice_no')}}</span></th>
                    <td><span>{{$invoice->invoice_no}}</span></td>
                </tr>
                @if(! empty($invoice->reference) )
                <tr>
                    <th><span>{{trans('quotes::custom.quotes.reference')}}</span></th>
                    <td><span>{{$invoice->reference}}</span></td>
                </tr>
                @endif
                <tr>
                ...
                <tr>
                    ...
                    <td><span id="prefix">{{digiCurrency( $total_paid, $invoice->currency_id )}}</span></td>
                </tr>
                <tr>
                    <th><span>{{trans('custom.invoices.amount-due')}}</span></th>
                    <td><span id="prefix">{{digiCurrency( $amount_due, $invoice->currency_id )}}</span></td>
                </tr>
            </table>
            <table class="inventory" style="margin-right: 90px; margin-top: -20px;">
                <thead>
                    <tr>
                        ...
                        <th><span>{{ trans('custom.products.quantity') }}</span></th>
                        <th><span>
            ...
            <?php } ?>
                <tr>
                    ...
                    $amount_due = $invoice->amount - $total_paid;
                    ?>
                        <span data-prefix>{{digiCurrency( $total_paid, $invoice->currency_id )}}</span></td>
                </tr>
                <tr>
                    <th><span>{{trans('custom.invoices.amount-due')}}</span></th>
                    <td><span data-prefix>{{digiCurrency( $amount_due, $invoice->currency_id )}}</span></td>
                </tr>
            </table>
        </article>
        <div class="row">
    ...
    <header>
            <address solox style="width:300px">
                ...
                @if(! empty( $invoice->terms_conditions ) )
                    ...
                    <code>{!! clean($invoice->terms_conditions) !!}</code>
                @endif
...
@if ( 'Yes' === $enable_signature_part )
  <div class="column" style="float:right; margin-right: 45px;">
    <header style="padding-left: 55px;">
            <address>
                ...
                <span style="margin-right: -100px; float:right; padding-top: 4px;padding-right: 0px;">
                <img alt="" src="{{asset( 'uploads/settings/' . $authorized_sign )}}" style=" width: 100px;"></span>
                @endif
                ...
                <span style="float: right;
                                                    ...
                                                    padding-right: 0px;
                                                    margin-left: 184px;
                                                    margin-right: 0px;">({{$authorized_person}})</span>
                @endif
                @if( ! empty( $authorized_designation ) )
                <br><span style="text-align:right; padding-top:15px;">{{$authorized_designation}}</span>
                @endif
            </address>
        </header>
</div>
